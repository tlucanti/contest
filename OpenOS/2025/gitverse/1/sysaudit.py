...
PID_CACHE = {}  # Cache PID -> process name (optional, can be extended)
...
class FileAuditWatcher(pyinotify.ProcessEvent):
    ...
    def __init__(
        ...
        self.git_helper.commit(f"Started tracking files\n\n - {body}")
...
def parse_args():
    ...
    drift_check_parser.add_argument(
        ...
        default="low",
        help="Severity level for drift detection. Default: low",
    )
    drift_check_parser.add_argument(
        "--repo",
        required=True,
    ...
    # metric-report command
    metric_report_parser = subparsers.add_parser(
        "metric-report",
        help="Generate a metrics report",
    )
...
def main():
    ...
    if args.command == "monitor":
        ...
        run_snapshot(args.message, args.repro)
    elif args.command == "drift-check":
        run_drift_check(args.baseline, args.severity, args.repro)
    elif args.command == "compliance-report":
        run_compliance_report(args.output)
    elif args.command == "roll-back":
        run_roll_back(args.to_commit, args.path, args.dry_run)
